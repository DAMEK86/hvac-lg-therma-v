...
#[derive(OpenApi)]
#[openapi(paths(get_coil, post_coil))]
...
pub fn create_router(hvac: String) -> Router {
    Router::new().nest(
        &format!("/{}", API_VERSION),
        Router::new()
            .route("/coils/:name", routing::get(get_coil))
            .route("/coils/:name", routing::post(post_coil))
            .with_state(hvac),
    )
}
...
async fn get_coil(Path(_name): Path<String>, State(hvac): State<String>) -> impl IntoResponse {
...
async fn post_coil(Path(_name): Path<String>, State(_hvac): State<String>) -> impl IntoResponse {
    not_found("Register not found".into(), None).into_response()
}
